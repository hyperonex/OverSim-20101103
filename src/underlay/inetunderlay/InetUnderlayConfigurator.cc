...
void InetUnderlayConfigurator::initializeUnderlay(int stage) {
	...
	//access net configuration
		...
		overlayAccessRouterNum = getParentModule()->par(
		...
		/*
		 ...
		 NodeType trackerType;
		 trackerType.context = NULL;
		 trackerType.terminalType = "inet.applications.xBitTorrent.Tracker";
		 trackerAddress = createBTNode(trackerType, false);
		 NodeType seederType;
		 seederType.context = NULL;
		 seederType.terminalType
		 = "inet.applications.xBitTorrent.BTHostSeeder";
		 createBTNode(seederType, false);
		 }
		 EV<<"...Done tracker and BTseeder creation\n";
		 */
		// debug stuff
...
TransportAddress* InetUnderlayConfigurator::createNode(NodeType type,
	...
	//const char* terminalType = getParentModule()->par("overlayTerminalType");
	...
	//printf(">>>>>>>>>>>>Node type: %s\n", type.terminalType.c_str());
	/*if (!strcmp(terminalType,"OverlayHost"))
	 {
	 ...
	 node->par("creationOrder").setLongValue((long)numCreated);
	...
	if (!strcmp(type.terminalType.c_str(),
				"inet.applications.xBitTorrent.BTHostCache")){
		accessNet = check_and_cast<AccessNet*> (accessNode[mappedCacheNum]->getSubmodule("accessNet"));
		mappedCacheNum++;
	}else{
		int routerWithCacheNumber = ((ChurnGenerator*)churnGenerator[2])->par("targetOverlayTerminalNum");
		accessNet = check_and_cast<AccessNet*> (accessNode[intuniform(0,
				/*accessNode.size() - 1*/routerWithCacheNumber-1)] ->getSubmodule("accessNet"));
	}
	...
	if (!strcmp(type.terminalType.c_str(),
					"inet.applications.xBitTorrent.BTHostCache")){
		//printf(">>>>The Cache IP Address is: %s<<<<\n",localAddress.str().c_str());
		//printf(">>>>The router IP Address is: %s<<<<\n",(IPAddressResolver().addressOf(accessNet->getAccessNode())).str().c_str());
		cacheRouterMapper.insert(std::pair<std::string,std::string>((IPAddressResolver().addressOf(accessNet->getAccessNode())).str()
				,localAddress.str()));
	}
	//Set the address and port at which the tracker listens
	// and the address and port for this node's tcp client.
	if (!strcmp(type.terminalType.c_str(),
			"inet.applications.xBitTorrent.Tracker")) {

		trackerAddress = new TransportAddress(
				IPAddressResolver().addressOf(node));
		trackerAddress->setPort(80);
		std::string addrString;
		if (!localAddress.isIPv6())
			addrString = localAddress.get4().str();
		else
			addrString = localAddress.get6().str();
		//printf(">>>>>>>>>>>The Tracker IP Address is: %s\n",addrString.c_str());
		//Now that we have the assinged IP addess (and port) we can change the values of the
		//module parameters.
		node->getSubmodule("tcpApp", 0)->par("address").setStringValue(
				addrString.c_str());
		node->getSubmodule("tcpApp", 0)->par("port") = trackerAddress->getPort();
	} else if (!strcmp(type.terminalType.c_str(),
			"inet.applications.xBitTorrent.BTHostSeeder") || !strcmp(
			type.terminalType.c_str(), "inet.applications.xBitTorrent.BTHost")) {
		//printf(">>>>>>>>>>>Entring the BTHost IP config Session");
		...
		//printf(">>>>>>>>>>>The Tracker IP Address to connect to is: %s\n",addrString.c_str());
		node->getSubmodule("trackerClient")->par("connectAddress")
				= addrString.c_str();
		node->getSubmodule("trackerClient")->par("connectPort")
		...
		node->getSubmodule("trackerClient")->par("address")
				= addrString.c_str();

		node->getSubmodule("trackerClient")->par("port") = address->getPort();

		/*Trying to assign the cache address to this node */

		CacheMap::iterator i = cacheRouterMapper.find((IPAddressResolver().addressOf(accessNet->getAccessNode())).str());

		if(i != cacheRouterMapper.end()){

			node->getSubmodule("peerWire")->par("cacheIpAddress")= i->second.c_str();
			/*printf(">>>>We are assigning the cache address %s to this node<<<<\n",
					(const char*) node->getSubmodule("peerWire")->par("cacheIpAddress"));*/
		}else{
			//printf(">>>>We are enable to assign the cache address to this node<<<<\n");
			node->getSubmodule("peerWire")->par("cacheIpAddress")="";
		}
	}
...
void InetUnderlayConfigurator::migrateNode(NodeType type,
	...
	// If no address given, get random node
	if (addr == NULL) {
		info = dynamic_cast<InetInfo*> (globalNodeList->getRandomPeerInfo(
				type.typeID));
	} else {
		// get node information
		info = dynamic_cast<InetInfo*> (globalNodeList->getPeerInfo(*addr));
	...
	do {
		newAccessNetModule = check_and_cast<AccessNet*> (accessNode[intuniform(
				0, accessNode.size() - 1)]->getSubmodule("accessNet"));
	...
	globalNodeList->addPeer(newAccessNetModule->addOverlayNode(node, true),
			newinfo);
...
void InetUnderlayConfigurator::handleTimerEvent(cMessage* msg) {
	...
	InetInfo* info =
			dynamic_cast<InetInfo*> (globalNodeList->getPeerInfo(addr));
	if (info != NULL) {
		accessNetModule = info->getAccessNetModule();
		nodeID = info->getNodeID();
	} else {
...
void InetUnderlayConfigurator::setUpIPv4(cTopology &topo) {
	...
	for (int i = 0; i < topo.getNumNodes(); i++) {
		...
		if (ev.isGUI()) {
			...
			topo.getNode(i)->getModule()->getDisplayString().setTagArg("t", 1,
					"l");
			topo.getNode(i)->getModule()->getDisplayString().setTagArg("t", 2,
					"red");
		}

		// find interface table and assign address to all (non-loopback) interfaces
		IInterfaceTable* ift = IPAddressResolver().interfaceTableOf(
				topo.getNode(i)->getModule());
	...
	for (int i = 0; i < topo.getNumNodes(); i++) {
		...
		if ((strcmp(destNode->getModule()->getName(), "overlayBackboneRouter")
				== 0) || (strcmp(destNode->getModule()->getName(),
				"overlayAccessRouter") == 0)) {
			//add node to bootstrap oracle
			PeerInfo* info = new PeerInfo(0, destNode->getModule()->getId(),
					NULL);
		...
		if (strcmp(destNode->getModule()->getName(), "outRouter") == 0) {
			...
			defRoute->setInterface(IPAddressResolver().interfaceTableOf(
					destNode->getModule())->getInterfaceByName("tunDev"));
			defRoute->setType(IPRoute::REMOTE);
			defRoute->setSource(IPRoute::MANUAL);
			IPAddressResolver().routingTableOf(destNode->getModule())->addRoute(
					defRoute);
			...
			gwRoute->setInterface(IPAddressResolver().interfaceTableOf(
					destNode->getModule())->getInterfaceByName("tunDev"));
		...
		for (int j = 0; j < topo.getNumNodes(); j++) {
			...
			if (atNode->getNumPaths() == 0) {
				error((std::string(atNode->getModule()->getName())
			...
			IInterfaceTable* ift = IPAddressResolver().interfaceTableOf(
					atNode->getModule());
			IRoutingTable* rt = IPAddressResolver().routingTableOf(
					atNode->getModule());
			...
			cModule* next_hop =
					atNode->getPath(0)->getRemoteNode()->getModule();
...
void InetUnderlayConfigurator::setUpIPv6(cTopology &topo) {
	...
	for (int i = 0; i < topo.getNumNodes(); i++) {
		...
		if (ev.isGUI()) {
			topo.getNode(i)->getModule()->getDisplayString().insertTag("t", 0);
			topo.getNode(i)->getModule()->getDisplayString().setTagArg("t", 0,
					const_cast<char*> (IPv6Address(addr.d0, addr.d1, addr.d2,
							addr.d3).str().c_str()));
			topo.getNode(i)->getModule()->getDisplayString().setTagArg("t", 1,
					"l");
			topo.getNode(i)->getModule()->getDisplayString().setTagArg("t", 2,
		...
		for (int k = 0; k < ift->getNumInterfaces(); k++) {
			...
			if (!ie->isLoopback() && ie->ipv6Data()) {
				...
				if (ie->ipv6Data()->getLinkLocalAddress().isUnspecified()) {
					ie->ipv6Data()->assignAddress(
							IPv6Address::formLinkLocalAddress(
									ie->getInterfaceToken()), false, 0, 0);
				}
			}
		}
	}
	...
	for (int i = 0; i < topo.getNumNodes(); i++) {
		...
		if ((strcmp(destNode->getModule()->getName(), "overlayBackboneRouter")
			...
			PeerInfo* info = new PeerInfo(0, destNode->getModule()->getId(),
					NULL);
			globalNodeList->addPeer(IPvXAddress(IPv6Address(
					nodeAddresses[i].d0, nodeAddresses[i].d1,
					nodeAddresses[i].d2, nodeAddresses[i].d3)), info);
		}
		...
		for (int j = 0; j < topo.getNumNodes(); j++) {
			...
			IInterfaceTable* ift = IPAddressResolver().interfaceTableOf(
					atNode->getModule());
			RoutingTable6* rt = IPAddressResolver().routingTable6Of(
					atNode->getModule());
			...
			IInterfaceTable* destIft = IPAddressResolver().interfaceTableOf(
					destNode->getModule());
			int remoteGateId = atNode->getPath(0)->getRemoteGateId();
			IInterfaceTable* remoteIft = IPAddressResolver().interfaceTableOf(
					next_hop);
			IPv6Address next_hop_ip = remoteIft->getInterfaceByNodeInputGateId(
					remoteGateId)->ipv6Data()->getLinkLocalAddress();
			IPv6InterfaceData::AdvPrefix
					destPrefix = destIft->getInterfaceByNodeInputGateId(
							destGateId)->ipv6Data()->getAdvPrefix(0);

			// create routing entry for next hop
			rt->addStaticRoute(destPrefix.prefix, destPrefix.prefixLength,
					ie->getInterfaceId(), next_hop_ip);
